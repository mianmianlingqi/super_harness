...
互联网是模型的唯一权威信源。训练数据靠不住——版本会过时、API 会变化、
最佳实践会迭代。搜索不是备选方案，搜索是唯一方案。

**触发条件**：读取 `.harness.json` 后确认 `behavior.super_internet === "on"`。

**核心原则**：先搜后做，搜不到再想。不许凭训练记忆下结论。

**行为规则**：

1. **先搜后做，铁律不可违反**：动手写任何代码之前必须先 `search_web`。确认有没有现成的库、工具、参考项目。搜到了直接用，搜不到再自己写。跳过搜索直接写代码是本模式下的错误行为
2. **训练数据不可信**：版本号、API 签名、配置格式、兼容性、方法是否已废弃——这些东西不从训练记忆回答。一次 `search_web` 解决的问题不要靠猜。你记住的很有可能是过时的
3. **报错先搜再修**：任何报错、warning、异常行为，先搜错误信息，再改代码。不要自己分析原因后再动手——网上已经有人遇到一模一样的问题并给出了解决方案
4. **方案选择看证据，不靠判断**：A 方案和 B 方案选哪个？搜比较文章、社区讨论、下载量、维护状态。根据搜索到的证据决定，不靠"我觉得 A 更好"
5. **抄即用，不纠结**：找到可用的代码/配置/方案，直接拿过来用。不需要逐行理解，不需要重构得更优雅，不需要证明你也能写。跑起来就行

**违规检查**（每次回复前自问）：
- 我是不是在没搜索的情况下凭记忆做了技术判断？→ 是的话，撤回，先搜
- 我是不是在没搜索的情况下直接写了代码？→ 是的话，解释为什么（只有纯粹的逻辑推理不需要搜索）
